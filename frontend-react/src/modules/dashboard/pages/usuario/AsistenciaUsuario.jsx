...
function AsistenciaUsuario() {
  ...
  useEffect(() => {
    const fetchAsistencias = async () => {
      ...
      try {
        ...
        // El backend ya obtiene el id_usuario del token, no es necesario enviarlo por query ni body
        const response = await fetch(`${API_BASE_URL}/api/asistencia/mia?filtro=${selectedPeriod}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (!response.ok) throw new Error('No se pudo cargar la asistencia');
        const data = await response.json();
        console.log('Asistencias recibidas:', data); // <-- log para depuración
        setAsistencias(data);
        setCurrentPage(1);
      } catch (err) {
  ...
  // Filtro por búsqueda (en tipo, fecha_hora o detalle)
  ...
  const currentAsistencias = filteredAsistencias.slice(indexOfFirstItem, indexOfLastItem);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col h-[calc(100vh-12rem)] bg-white rounded-lg shadow-sm">
        <div className="flex-1 p-6 overflow-auto min-h-0">
          {loading && <div className="text-center text-gray-500 mt-8">Cargando asistencia...</div>}
          {error && <div className="text-center text-red-500 mt-8">Error: {error}</div>}
          {!loading && !error && filteredAsistencias.length === 0 && (
            <div className="text-center text-gray-500 mt-8">No hay registros de asistencia.</div>
          )}
          {!loading && !error && filteredAsistencias.length > 0 && (
        ...
        <div className="border-t border-gray-200 p-4 bg-white">
          <div className="flex justify-between items-center">
            <div className="text-sm text-gray-700">
              Mostrando {indexOfFirstItem + 1} a {Math.min(indexOfLastItem, filteredAsistencias.length)} de {filteredAsistencias.length} resultados
            </div>
